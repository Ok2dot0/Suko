            ...
            ui.horizontal_wrapped(|ui| {
                ...
                if ui.button(egui::RichText::new("Save Session…").strong()).on_hover_text("Save session markdown to a file").clicked() {
                    if !self.steps.is_empty() {
                        ...
                        if let Some(path) = rfd::FileDialog::new()
                            ...
                            .save_file() {
                            // Write to chosen file path directly
                            let _ = write_session_markdown(path.parent().unwrap_or_else(|| std::path::Path::new(".")), &log)
                                .map(|p| {
                                    // If write_session_markdown wrote elsewhere, also write directly to chosen file
                                    let _ = fs::write(&path, format_session_markdown(&log));
                                    p
                                });
                        }
                    }
                ...
                if ui.button(egui::RichText::new("Save Board…").strong()).on_hover_text("Save current grid as 81-char .sdk").clicked() {
                    if let Some(path) = rfd::FileDialog::new()
                        ...
                        .save_file() {
                        let _ = fs::write(path, board_to_sdk(&self.board));
                    }
                }
    ...
    egui::Grid::new("board").num_columns(9).spacing([4.0, 4.0]).show(ui, |ui| {
        ...
        for r in 0..9 {
            for c in 0..9 {
                ...
                let mut button = egui::Button::new(text).min_size(egui::vec2(32.0, 32.0));
